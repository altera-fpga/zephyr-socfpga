			     ...
			     mm_reg_t perpllc_reg)
{
	...
	case CLKCTRL_CLKSRC_MAIN:
		clock = get_ref_clk(CLKCTRL_MAINPLL(PLLGLOB), CLKCTRL_MAINPLL(PLLM));
	...
	case CLKCTRL_CLKSRC_PER:
		clock = get_ref_clk(CLKCTRL_PERPLL(PLLGLOB), CLKCTRL_PERPLL(PLLM));
	...
	case CLKCTRL_CLKSRC_OSC1:
		clock = sys_read32(SOCFPGA_SYSMGR(BOOT_SCRATCH_COLD_1));
		break;

	case CLKCTRL_CLKSRC_INTOSC:
		clock = CLKCTRL_INTOSC_HZ;
		break;

	case CLKCTRL_CLKSRC_FPGA:
		clock = sys_read32(SOCFPGA_SYSMGR(BOOT_SCRATCH_COLD_2));
		break;
	...
	LOG_DBG("%s: clock source %lu and its value %u\n",
		__func__, GET_CLKCTRL_CLKSRC(clk_psrc), clock);
...
uint32_t get_mpu_clk(void)
{
	...
	if (cpu_id > CLKCTRL_CPU_ID_CORE1) {
		clock = get_clk_freq(CLKCTRL_CTLGRP(CORE23CTR),
				     CLKCTRL_MAINPLL(PLLC0),
				     CLKCTRL_PERPLL(PLLC0));
	} else {
		clock = get_clk_freq(CLKCTRL_CTLGRP(CORE01CTR),
	...
	clock /= 1 + (sys_read32(ctr_reg) & CLKCTRL_PLLCX_DIV_MSK);
